...
class PermsService(BaseService):
    ...
    def list(self, limit: int = 10, offset=None, search=None):
    ...
    def add_users_relations(self, reqs: list[AssetPermUserRelation]):
        ...
        items = [from_dict(AssetPermUserRelationDetail, item) for item in (data or [])]
        return items, resp
    ...
    def add_user_groups_relations(self, reqs: list[AssetPermUserGroupRelation]):
        ...
        items = [from_dict(AssetPermUserGroupRelationDetail, item) for item in (data or [])]
        return items, resp
    ...
    def add_assets_relations(self, reqs: list[AssetPermAssetRelation]):
        ...
        items = [from_dict(AssetPermAssetRelationDetail, item) for item in (data or [])]
        return items, resp
    ...
    def add_nodes_relations(self, reqs: list[AssetPermNodeRelation]):
        ...
        items = [from_dict(AssetPermNodeRelationDetail, item) for item in (data or [])]
        return items, resp
...
class SelfService(BaseService):
    ...
    def list_assets(self, filters: dict = None, limit: int = 10, offset=None, search=None):
        params = {}
        if filters:
        ...
        return [from_dict(SelfAsset, item) for item in results], resp

    def get_asset(self, asset_id: str):
        data, resp = self._client.get(f"/api/v1/perms/users/self/assets/{asset_id}/")
        return from_dict(SelfAssetDetail, data) if data else None, resp
